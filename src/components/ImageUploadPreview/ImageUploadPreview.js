...
export default function ImageUploadPreview({
  ...
  async function compressBase64AndResize(base64URL) {
    ...
    const compressedAndResizedImage = await compress.compress(
      [Compress.convertBase64ToFile(await getBase64(base64URL))],
      {
        maxWidth: 64,
        maxHeight: 64,
      }
  ...
  const handleFileUpload = async (e) => {
    ...
    filesAsArray.forEach((file, i) => {
      ...
      const base64FileResizedPromise = compressImageAndConvertToBase64(file, {
        maxWidth: 64,
        maxHeight: 64,
      })
  ...
  return (
    <Form.Group className="mb-3 preview" controlId="formBasicImages">
      ...
      />
      {imageData.imagePreview.length > 1 && (
        <div style={{ marginTop: "10px" }}>
          Use as setinhas para ordenar as imagens
        </div>
      )}
      <Container>
        <Row>
          {imageData.imagePreview.length > 0 &&
            imageData.imagePreview.map((image, i) => {
              return (
                <Col
                  key={i}
                  style={{
                    ...
                    >
                      {i !== 0 && (
                        <IoArrowUpCircleSharp
                          ...
                          style={{ left: "8px" }}
                          onClick={() => moveImage(i, moveUp)}
                        />
                      )}
                      <Card.Text
                        style={{ textAlign: "center", marginTop: "0.75rem" }}
                      >
                        {i === 0 ? "Foto de capa" : `Foto ${i}`}
                      </Card.Text>
                      {i < imageData.imagePreview.length - 1 && (
                        <IoArrowDownCircleSharp
                          className="arrow-button"
                          style={{ right: "8px" }}
                          onClick={() => moveImage(i, moveDown)}
                        />
                      )}
                    </div>
                  </Card>
